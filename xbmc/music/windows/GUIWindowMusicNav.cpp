...
bool CGUIWindowMusicNav::OnMessage(CGUIMessage& message) {
	...
	case GUI_MSG_PLAYBACK_STARTED: {
		SET_CONTROL_SELECTED(GetID(), CONTROL_BTNPARTYMODE,
				g_partyModeManager.IsEnabled());
	}
		break;
...
CStdString CGUIWindowMusicNav::GetQuickpathName(
		const CStdString& strPath) const {
	if (strPath.Equals("musicdb://1/"))
		...
		CLog::Log(LOGERROR,
				"  CGUIWindowMusicNav::GetQuickpathName: Unknown parameter (%s)",
				strPath.c_str());
		return strPath;
	}
}
...
bool CGUIWindowMusicNav::GetDirectory(const CStdString &strDirectory,
		CFileItemList &items) {
	if (m_bDisplayEmptyDatabaseMessage)
		return true;
	...
	if (strDirectory.Left(10).Equals("videodb://")) {
		...
		VIDEODATABASEDIRECTORY::NODE_TYPE node = dir.GetDirectoryChildType(
				strDirectory);
	...
	if (iItems) {
		...
		for (int i = 0; i <= (iItems >= 2 ? 1 : 0); i++) {
			...
			if (pItem->IsParentFolder())
				iItems--;
			if (pItem->GetPath().Left(4).Equals("/-1/"))
				iItems--;
		}
		// or the last item
		if (m_vecItems->Size() > 2
				&& m_vecItems->Get(m_vecItems->Size() - 1)->GetPath().Left(4).Equals(
						"/-1/"))
			iItems--;
	}
	CStdString items;
...
void CGUIWindowMusicNav::OnWindowLoaded() {
	...
	m_searchWithEdit = (control
			&& control->GetControlType() == CGUIControl::GUICONTROL_EDIT);

	CGUIWindowMusicBase::OnWindowLoaded();

	if (m_searchWithEdit) {
		SendMessage(GUI_MSG_SET_TYPE, CONTROL_SEARCH,
				CGUIEditControl::INPUT_TYPE_SEARCH);
		SET_CONTROL_LABEL2(CONTROL_SEARCH, GetProperty("search").asString());
	}
}

void CGUIWindowMusicNav::GetContextButtons(int itemNumber,
		CContextButtons &buttons) {
	CGUIWindowMusicBase::GetContextButtons(itemNumber, buttons);

	CGUIDialogMusicScan *musicScan =
			(CGUIDialogMusicScan *) g_windowManager.GetWindow(
	...
	if (item && (item->GetExtraInfo().Find("lastfm") < 0)) {
		...
		bool inPlaylists = m_vecItems->GetPath().Equals(
		...
		if (item->IsAudio() && !item->IsPlayList() && !item->IsSmartPlayList()
			...
			if (item->GetPath().Left(14).Equals("videodb://3/4/")
				...
				long idArtist = m_musicdatabase.GetArtistByName(
						m_vecItems->Get(itemNumber)->GetLabel());
				if (idArtist > -1)
					buttons.Add(CONTEXT_BUTTON_INFO, 21891);
			}
		} else if (!inPlaylists
				&& (dir.HasAlbumInfo(item->GetPath())
		...
		if (!item->IsParentFolder() && !inPlaylists
				&& (nodetype == NODE_TYPE_ROOT || nodetype == NODE_TYPE_OVERVIEW
		...
		if (item->HasMusicInfoTag()
				&& item->GetMusicInfoTag()->GetArtist().size() > 0) {
			CVideoDatabase database;
			database.Open();
			if (database.GetMatchingMusicVideo(item->GetMusicInfoTag()->GetArtist())
					> -1)
		...
		if (inPlaylists
				&& !URIUtils::GetFileName(item->GetPath()).Equals("PartyMode.xsp")
...
bool CGUIWindowMusicNav::OnContextButton(int itemNumber,
	...
	case CONTEXT_BUTTON_INFO: {
		...
		if (item->HasVideoInfoTag()
				&& !item->GetVideoInfoTag()->m_strTitle.IsEmpty()) {
			CGUIWindowVideoNav* pWindow =
					(CGUIWindowVideoNav*) g_windowManager.GetWindow(WINDOW_VIDEO_NAV);
			if (pWindow) {
				ADDON::ScraperPtr info;
	...
	case CONTEXT_BUTTON_GO_TO_ARTIST: {
		...
		CVideoDatabase database;
		database.Open();
		strPath.Format("videodb://3/4/%ld/",
				database.GetMatchingMusicVideo(item->GetMusicInfoTag()->GetArtist()));
	...
	case CONTEXT_BUTTON_SET_CONTENT: {
		...
		if (m_vecItems->GetPath().Equals("musicdb://1/")
				|| item->GetPath().Equals("musicdb://2/")) {
			content = CONTENT_ARTISTS;
		}
		...
		if (!m_musicdatabase.GetScraperForPath(path, scraper,
			...
			if (ADDON::CAddonMgr::Get().GetDefault(
					ADDON::ScraperTypeFromContent(content), defaultScraper)) {
				scraper = boost::dynamic_pointer_cast < ADDON::CScraper
						> (defaultScraper->Clone(defaultScraper));
			}
		}
...
bool CGUIWindowMusicNav::GetSongsFromPlayList(const CStdString& strPlayList,
		CFileItemList &items) {
	...
	CLog::Log(LOGDEBUG, "CGUIWindowMusicNav, opening playlist [%s]",
			strPlayList.c_str());
...
void CGUIWindowMusicNav::FrameMove() {
	...
	if (m_searchTimer.IsRunning()
			&& m_searchTimer.GetElapsedMilliseconds() > search_timeout) {
		OnSearchUpdate();
		m_searchTimer.Stop();
	}
	if (m_bDisplayEmptyDatabaseMessage)
